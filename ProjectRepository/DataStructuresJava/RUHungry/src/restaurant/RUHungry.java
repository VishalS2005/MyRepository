...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        String[] cVar = new String[size];
        MenuNode[] mVar = new MenuNode[size];
        for (int i = 0; i < size; i++) {
            cVar[i] = StdIn.readLine();
            int dishSize = StdIn.readInt();
            StdIn.readLine();
            for (int j = 0; j < dishSize; j++) {
                String dish = StdIn.readLine();
                int[] tempID = new int[StdIn.readInt()];
                for (int k = 0; k < tempID.length; k++) {
                    tempID[k] = StdIn.readInt();
                }
                StdIn.readLine(); // \n
                Dish d = new Dish(cVar[i], dish, tempID);
                MenuNode m = new MenuNode(d, null);
                m.setNextMenuNode(mVar[i]);
                mVar[i] = m;
            }
        }
        categoryVar = cVar;
    ...
    public void addStockNode ( StockNode newNode ) {
        int index = (newNode.getIngredient().getID() % (stockVar.length));
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if (ingredientName == null) {
            temp = findStockNode(ingredientID);
            old = temp;
            temp.getIngredient().updateStockLevel(stockAmountToAdd);
        } else if (ingredientID == -1) {
            temp = findStockNode(ingredientName);
            old = temp;
            temp.getIngredient().updateStockLevel(stockAmountToAdd);
        } else {
            temp = findStockNode(ingredientName);
        ...
        for ( int index = 0; index < stockVar.length; index++){
            ...
            while ( ptr != null ){
                if (ptr.equals(old)){
                    ...
                    prev.setNextStockNode(temp);
                    }
                    temp.setNextStockNode(ptr.getNextStockNode());
                    break;
                }  
                    prev = ptr;
                    ptr = ptr.getNextStockNode();
    ...
    public void updatePriceAndProfit() {
        ...
        for ( int category = 0; category < menuVar.length; category++ ) {
            MenuNode ptr = menuVar[category]; // set ptr at the front (first menuNode)
            while ( ptr != null ) { // while loop that searches the LL of the category to find the itemOrdered
                int[] temp = ptr.getDish().getStockID();
                double cost = 0;
                for (int i = 0; i < temp.length; i++) {
                    StockNode s = findStockNode(temp[i]);
                    cost += s.getIngredient().getCost();
                }
                ptr.getDish().setPriceOfDish(cost*1.2);
                ptr.getDish().setProfit(ptr.getDish().getPriceOfDish() - cost);
                ptr = ptr.getNextMenuNode();
            }
        }
	// WRITE YOUR CODE HERE
    ...
    public void createStockHashTable (String inputFile){
        ...
        int stockSize = Integer.parseInt(StdIn.readLine());
        StockNode[] temp = new StockNode[stockSize];
        stockVar = temp;
        while(!StdIn.isEmpty()) {
            int tempID = StdIn.readInt();
            StdIn.readChar();
            String tempName = StdIn.readLine();
            Double tempPrice = StdIn.readDouble();
            int tempLevel = StdIn.readInt();
            Ingredient tempIng = new Ingredient(tempID, tempName, tempLevel, tempPrice);
            StockNode tempStock = new StockNode(tempIng, null);
            addStockNode(tempStock);
        }
	// WRITE YOUR CODE HERE
	...
	// WRITE YOUR CODE HERE
        int[] temp = findDish(dishName).getDish().getStockID();
        for (int i = 0; i < temp.length; i++) if(numberOfDishes > findStockNode(temp[i]).getIngredient().getStockLevel()) return false;
     ...
     *          - you might have to traverse through the entire category searching for a dish that can be prepared. If you reach the end of the list, 
     *            start from the beginning until you have visited EVERY dish in the category.
	...
	// WRITE YOUR CODE HERE
        if(checkDishAvailability(dishName, quantity)) {
            ...
            int[] temp = findDish(dishName).getDish().getStockID();
            for (int i = 0; i < temp.length; i++) updateStock(null, temp[i], -1 * quantity);
        } else {
            ...
            while(ptr != findDish(dishName)) {
                if (ptr == null) ptr = menuVar[categoryIndex]; 
                if (checkDishAvailability(ptr.getDish().getName(), quantity)) {
                    TransactionData tempData = new TransactionData("order", ptr.getDish().getName(), quantity, ptr.getDish().getProfit()*quantity, true);
                    addTransactionNode(tempData);
                    int[] temp = ptr.getDish().getStockID();
                    for (int i = 0; i < temp.length; i++) updateStock(null, temp[i], -1 * quantity);
                    break;
                } else {
                    TransactionData tempData = new TransactionData("order", ptr.getDish().getName(), quantity, 0, false);
                    addTransactionNode(tempData);
                    ptr = ptr.getNextMenuNode();
                    if (ptr == null) ptr = menuVar[categoryIndex]; 
                }
    ...
    public void donation (String ingredientName, int quantity){
        ...
        if (profit > 50.0 && (findStockNode(ingredientName).getIngredient().getStockLevel() - quantity >= 0))  {
